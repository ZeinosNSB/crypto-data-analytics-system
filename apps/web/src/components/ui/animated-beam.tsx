  ...
  useEffect(() => {
    const updatePath = () => {
      if (containerRef.current && fromRef.current && toRef.current) {
        ...
        const length = chord + Math.abs(curvature) * 0.6

        const pps = speed ?? 400 / Math.max(duration, 0.1)
        const next = Math.min(maxDuration, Math.max(minDuration, length / pps))
        setComputedDuration(next)
      }
    }
